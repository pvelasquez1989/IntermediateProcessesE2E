package co.com.bancolombia.certificacion.intermediateprocesses.stepdefinitions.regimechanges;

import static co.com.bancolombia.certificacion.intermediateprocesses.exceptions.EventReasonsException.ERROR_EVENT_REASON_REGIMEN_CHANGE;
...
public class RegimeChangeStepDefinition {
	...
	@Given("^that the (.*) needs to make an regime change to an employee in HCM (.*)$")
	public void thatTheAnalystNeedsToMakeAnRegimeChangeToAnEmployeeInHCM(String actorName, String scenarioNumber) {
...
//		theActorInTheSpotlight().wasAbleTo(executeCBTATest(scenarioNumber));
	...
	public void heValidateThatRegimeChangeCOIsRegisteredInSSFFSuccessfully() {
		theActorInTheSpotlight().should(seeThat(ResultEventReason.registeredIs(), equalTo(getExpectedResult()))
				.orComplainWith(EventReasonsException.class, ERROR_EVENT_REASON_REGIMEN_CHANGE));
	}
